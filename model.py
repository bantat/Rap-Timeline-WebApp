...
def buildPageBasedOnParameters(cgi_parameters):
    ...
    # For each possible parameter, checks to see if parameter exists...
    if cgi_parameters['year'] != '':
        ...
        if int(year) in data_source.getYearsOnTimeline():
            # And if it is, builds corresponding page using content generated by getSomeContent() methods
...
def getAlbumContent(album_name):
    ...
    content_dictionary = {'album_id':'','album_name':'','summary':'','image':'','year':'','artist_id':'',
                          'artist_name':'','spotify':''}
    content_dictionary['album_id'] = album_object.getAlbumName()
    content_dictionary['album_name'] = album_object.getAlbumString()
